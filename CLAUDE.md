...
- **Input**: `kind` (required): `"orphans"`, `"unresolved_links"`, or `"broken_anchors"`. `limit` (optional): Cap on the number of returned rows/groups (default `100`; pass `0` for unbounded — no cap).
