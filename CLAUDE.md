...
- **Input**:
  - `select` (required): either `paths` (explicit array of note paths) **or** a filter — any combination of `where` (query_notes-style condition object), `tags` (find_by_tag-style), and `folder` (path prefix). A `folder` on its own is a valid filter (it selects every note under that folder); the three are the shared candidate-filter vocabulary, so no one of them is privileged as "the" filter. `match` (`"any"` default or `"all"`) governs how multiple `tags` combine — the primary filter only, matching every other note-selecting tool; `where` conditions always combine as `all` (its own default, as a secondary filter does on `find_by_tag`). An optional `limit` caps matched notes (`0` = unbounded, following the vault-wide convention). Exactly one of `paths` or the filter form must be given — providing both errors, and providing neither errors.
